...
import { getFileIcon } from "@/lib/structure"
import { CommentDialog } from "../dialogs/comment-dialog"
import type { FileItem, ClipboardItem } from "@/types/index"
...
const FileFolder: React.FC<FileFolderProps> = ({
    ...
    const isInitialFocusRef = useRef<boolean>(false) // Track if we're in initial focus mode
    ...
    useEffect(() => {
        console.log(`[DEBUG] useEffect triggered - isEditing: ${isEditing}, item.name: ${item.name}, item.id: ${item.id}`)
        
        if (isEditing && inputRef.current) {
            console.log(`[DEBUG] Starting edit mode for: ${item.name}`)
            // Set flag to prevent blur during initial focus
            isInitialFocusRef.current = true
            
            // Sync editName with item.name when editing starts
            setEditName(item.name)
            
            // Use setTimeout to ensure the state update completes before focusing
            setTimeout(() => {
                console.log(`[DEBUG] setTimeout callback - focusing input for: ${item.name}`)
                if (inputRef.current) {
                    inputRef.current.focus()
                    inputRef.current.select()
                    console.log(`[DEBUG] Input focused and selected for: ${item.name}`)
                    
                    // Set initial width based on content
                    if (measureRef.current) {
                        const width = measureRef.current.offsetWidth
                        setInputWidth(`${width + 50}px`) // Add some padding
                    }
                } else {
                    console.log(`[DEBUG] inputRef.current is null!`)
                }
            }, 10)
        } else if (!isEditing) {
            // Clear flag when editing ends
            isInitialFocusRef.current = false
        }
    }, [isEditing, item.name])

    const handleBlur = () => {
        console.log(`[DEBUG] handleBlur called - isInitialFocusRef: ${isInitialFocusRef.current}, item.name: ${item.name}`)
        
        // If we're still in initial focus mode, refocus the input
        if (isInitialFocusRef.current) {
            console.log(`[DEBUG] handleBlur - refocusing input (initial focus mode)`)
            // Refocus after a microtask to prevent the blur from taking effect
            setTimeout(() => {
    ...
    // Clear initial focus flag on first keypress
    const handleKeyDown = (e: React.KeyboardEvent) => {
        // Clear initial focus mode on any key press (except modifier keys)
        if (isInitialFocusRef.current && !['Shift', 'Control', 'Alt', 'Meta'].includes(e.key)) {
            console.log(`[DEBUG] Clearing isInitialFocusRef on keypress: ${e.key}`)
            isInitialFocusRef.current = false
        }
    ...
    const handleRename = () => {
        console.log(`[DEBUG] handleRename called - isInitialFocusRef: ${isInitialFocusRef.current}, item.name: ${item.name}`)
        
        // Skip if we're still in initial focus mode
        if (isInitialFocusRef.current) {
            console.log(`[DEBUG] handleRename SKIPPED (initial focus mode)`)
            return
        }
        ...
        console.log(`[DEBUG] handleRename - setting currentEditingId to null`)
        setCurrentEditingId(null)
    }
    ...
    const handleClick = (e: React.MouseEvent) => {
        console.log(`[DEBUG] handleClick called - item.name: ${item.name}, isInitialFocusRef: ${isInitialFocusRef.current}, currentEditingId: ${currentEditingId}`)
        
        // Don't process click events during initial editing focus
        if (isInitialFocusRef.current) {
            console.log(`[DEBUG] handleClick BLOCKED (initial focus mode)`)
            e.stopPropagation()
            return
        }
        
        if (currentEditingId && currentEditingId !== item.id) {
            console.log(`[DEBUG] handleClick - calling handleRename because editing different item`)
            handleRename()
        }

        // Check if this is a multi-select action
        ...
        // Toggle folder ONLY on normal click (not multi-select)
        if (item.type === "folder" && !isMultiSelect && !isEditing) {
            toggleFolder()
        }
    }
