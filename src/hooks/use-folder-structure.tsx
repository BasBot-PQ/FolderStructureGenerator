...
import type { FileItem, ClipboardItem, StructurePreviewDialogProps } from "@/types/index"
import {
...
import { INITIAL_STRUCTURE } from "@/constants/initial-structure"
import { useHistory } from "./use-history"
import { v4 as uuidv4 } from 'uuid'
import { STORAGE_KEYS } from "@/constants/storage-keys"
import { usePreviewFormat } from "./use-preview-format"
...
export const useFolderStructure = (tabId?: string) => {
    ...
    // Computed values are generated on-demand via getPreviewContent()
    ...
    // Helper function to find insertion index for alphabetical order
    const findAlphabeticalInsertIndex = useCallback((children: FileItem[], newName: string): number => {
        // Sort order: A-Z (case-insensitive), then 0-9
        for (let i = 0; i < children.length; i++) {
            if (newName.localeCompare(children[i].name, undefined, { numeric: true, sensitivity: 'base' }) < 0) {
                return i
            }
        }
        return children.length // Insert at end if name is last alphabetically
    }, [])
    ...
    const onAdd = useCallback(
        (parentId: string, name: string, type: "file" | "folder") => {
            ...
            updateStructure((prev) => {
                const addToItem = (item: FileItem): FileItem => {
                    if (item.id === parentId) {
                        ...
                        if (item.isManuallyOrdered) {
                            // If manually ordered, just append at the end
                            newChildren = [...existingChildren, newItem]
                        } else {
                            // Insert in alphabetical order
                            const insertIndex = findAlphabeticalInsertIndex(existingChildren, uniqueName)
                            newChildren = [
            ...
            console.log(`[DEBUG] onAdd - newItemId: ${newItemId}, type: ${type}`)

            if (type === "folder" && newItemId) {
                setOpenFolders((prev) => new Set([...prev, newItemId!]))
            }

            // Auto-start editing the new item after DOM renders
            if (newItemId) {
                console.log(`[DEBUG] onAdd - scheduling setCurrentEditingId after 50ms`)
                // Use setTimeout to ensure the new item is rendered before focusing
                setTimeout(() => {
                    console.log(`[DEBUG] onAdd - now calling setCurrentEditingId(${newItemId})`)
        ...
        (e: React.DragEvent, targetId: string, position: "before" | "after" | "inside" = "inside") => {
            ...
            try {
                ...
                updateStructure((prev) => {
                    ...
                    // Mark the target parent as manually ordered
                    const markAsManuallyOrdered = (item: FileItem): FileItem => {
                        if (item.id === targetParentId) {
                            return { ...item, isManuallyOrdered: true }
                        }
                        if (item.children) {
                            return { ...item, children: item.children.map(markAsManuallyOrdered) }
                        }
                        return item
                    }
                    newStructure = markAsManuallyOrdered(newStructure)
    ...
    // Preview Format (using extracted hook)
    const { format: previewFormat, setFormat: setPreviewFormat } = usePreviewFormat()
